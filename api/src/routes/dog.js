...
router.get("/", async (req, res, next) => {
  try {
    ...
    if (name) {
      let dogNameFilter = DogNameAll.filter((e) =>
        e.name.toLowerCase().includes(name.toLowerCase())
      );
      dogNameFilter.length > 0
        ? res.status(200).send(dogNameFilter)
        : res.status(200).send("no se encontro perro con ese nombre");
    } else {
...
router.get("/:id", async (req, res, next) => {
  try {
    ...
    let dogFilter = DogIdAll.filter((e) => id == e.id);
    dogFilter.length
      ? res.status(200).send(dogFilter)
      : res.status(200).send("no se encontro perro con esa id");
  } catch (error) {
    console.error(error);
    return [];
  }
...
router.post("/", async (req, res, next) => {
  ...
  let tempArray = [];
  let nameMayusc = name.toLowerCase().split("");
  nameMayusc[0] = nameMayusc[0].toUpperCase();
  name = nameMayusc.join("");
  let breedMayusc = breed.toLowerCase().split("");
